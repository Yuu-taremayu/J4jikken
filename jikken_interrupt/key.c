...
// 一定時間（数ms程度）毎に key_sense() を呼び出すことが前提
     ...
     /*      離されている，押されている，今押された，今離された，キーなし      */
{
  ...
  if ((keynum < KEYMINNUM) || (keynum > KEYMAXNUM))
    ...
    /* keynewvalとkeyoldvalを見てkeyvalを決める */
    keyoldval[keynum] = keynewval[keynum];
    keynewval[keynum] = key_check(keynum);
    switch (keynewval[keynum]) {      
    case KEYOFF:
      if (keyoldval[keynum] == KEYOFF) {
	keyval = KEYOFF;
      }
      else if (keyoldval[keynum] == KEYTRANS) {
	keyval = KEYNEGEDGE;
      }
      else if (keyoldval[keynum] == KEYON) {
	keyval = KEYNEGEDGE;
      }
      break;
      
    case KEYON:
      if (keyoldval[keynum] == KEYON) {
	keyval = KEYON;
      }
      else if (keyoldval[keynum] == KEYTRANS) {
	keyval = KEYPOSEDGE;
      }
      else if (keyoldval[keynum] == KEYOFF) {
	keyval = KEYPOSEDGE;
      }
      break;
